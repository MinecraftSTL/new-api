...
func decideTaskRetry(c *gin.Context, taskErr *taskdto.TaskError, retryTimes int) service.PolicyDecision {
	...
	// The configured retry status rules stay authoritative, including 000 for
	// responses without billable usage and the gateway-timeout codes.
	case operation_setting.ShouldRetryByError(types.ErrorCode(taskErr.Code), taskErr.StatusCode):
		return retry
	default:

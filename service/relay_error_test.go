...
func TestDecideRelayRetryReasons(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "gateway timeout stays outside the default retry rules", err: upstream(http.StatusGatewayTimeout), retries: 1, want: PolicyDecision{Action: "stop", Reason: "status_not_retryable", Source: "global"}},
		{name: "success status stays outside the default retry rules", err: upstream(http.StatusOK), retries: 1, want: PolicyDecision{Action: "stop", Reason: "status_not_retryable", Source: "global"}},

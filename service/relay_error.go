...
func DecideRelayRetry(c *gin.Context, err *types.NewAPIError, retryTimes int) PolicyDecision {
	...
	// The configured retry status rules stay authoritative: they carry the
	// success-status exclusion, the gateway-timeout exclusions, and the 000
	// pseudo status that marks responses without billable usage.
	if operation_setting.ShouldRetryByError(err.GetErrorCode(), err.StatusCode) {

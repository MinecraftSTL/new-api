...
func settleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) bool {
	if bc := task.PrivateData.BillingContext; bc != nil && bc.TieredSnapshot != nil {
		...
		actualQuotaBeforeGroup, beforeGroupClamp := common.QuotaRoundChecked(result.ActualQuotaBeforeGroup)
		RecalculateTaskQuotaWithBase(ctx, task, result.ActualQuotaAfterGroup, actualQuotaBeforeGroup, "任务用量表达式结算", result.Clamp, beforeGroupClamp)
		return true
	}
